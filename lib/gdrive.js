...
// 폴더 생성(기본 내 드라이브 루트). {ok,id,name,link}
async function createFolder({ userEmail, name, parents }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  if (!userEmail) return { ok: false, error: 'no_email' };
  try {
    const drive = driveFor(userEmail);
    const res = await drive.files.create({
      requestBody: { name: safeName(name), mimeType: 'application/vnd.google-apps.folder', parents: parents || ['root'] },
      fields: 'id,name,webViewLink',
    });
    return { ok: true, id: res.data.id, name: res.data.name, link: res.data.webViewLink };
...
// 구글 문서(텍스트 기반 — 편집→export 왕복 파싱에 유리). {ok,id,name,link}
async function createDocText({ userEmail, name, text, parents }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  if (!userEmail) return { ok: false, error: 'no_email' };
  try {
    const drive = driveFor(userEmail);
    const res = await drive.files.create({
      requestBody: { name: safeName(name), mimeType: 'application/vnd.google-apps.document', parents: parents || ['root'] },
      media: { mimeType: 'text/plain', body: '﻿' + String(text || '') },
      fields: 'id,name,webViewLink',
    });
    return { ok: true, id: res.data.id, name: res.data.name, link: res.data.webViewLink };
  } catch (e) { return { ok: false, error: (e && e.message) || 'unknown' }; }
...
// 구글 문서 내용 전체 교체(텍스트로 재변환). {ok}
async function updateDocText({ userEmail, fileId, text }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  try {
    const drive = driveFor(userEmail);
    await drive.files.update({ fileId, media: { mimeType: 'text/plain', body: '﻿' + String(text || '') } });
    return { ok: true };
  } catch (e) { return { ok: false, error: (e && e.message) || 'unknown' }; }
}

// 구글 문서 → 텍스트 추출(export). {ok, text}
async function exportDocText({ userEmail, fileId }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  try {
    const drive = driveFor(userEmail);
    const res = await drive.files.export({ fileId, mimeType: 'text/plain' }, { responseType: 'text' });
    return { ok: true, text: typeof res.data === 'string' ? res.data : String(res.data || '') };
  } catch (e) { return { ok: false, error: (e && e.message) || 'unknown' }; }
}

// 파일 존재 확인(휴지통 제외). {ok, exists}
async function exists({ userEmail, fileId }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  try {
    const drive = driveFor(userEmail);
    const res = await drive.files.get({ fileId, fields: 'id,trashed' });
    return { ok: true, exists: !!res.data && !res.data.trashed };
  } catch (e) { return { ok: true, exists: false }; }
}

module.exports = { configured, createDoc, createSheet, createSlides, search, copy, uploadRaw, createFolder, createDocText, updateDocText, exportDocText, exists };
