...
async function buildForUser({ userId, email, greeting = true, weather = '', motivation = '' }) {
  const d = kstNow();
  ...
  // 할 일
  let td = [];
  try { td = await todos.list(userId, email); } catch (e) { console.error('브리핑 할일 조회 실패:', e && e.message); }
  if (td.length) {
    lines.push(`\n✅ 할 일 ${td.length}건`);
