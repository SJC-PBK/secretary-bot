...
// 위임 메일함 목록 (env 쉼표구분). 조직개편 시 이 한 줄만 바꾸면 됨.
function accounts() {
  const raw = process.env.SECBOT_MAIL_ACCOUNTS || process.env.SECBOT_MAIL_ACCOUNT || '';
  return raw.split(',').map((s) => s.trim()).filter(Boolean);
}

function configured() {
  return fs.existsSync(KEY_PATH) && accounts().length > 0;
}

// 힌트(예: "sjc","ost")로 허용된 메일함 하나 선택. 힌트 없거나 불명확하면 첫 번째.
function resolveAccount(hint) {
  const list = accounts();
  if (!list.length) return '';
  if (!hint) return list[0];
  const h = String(hint).toLowerCase();
  return list.find((a) => a.toLowerCase().includes(h)) || list[0];
}

function gmailFor(acc) {
  const subject = acc || accounts()[0];
...
async function listRecent({ query, max = 10, account } = {}) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  try {
    const g = gmailFor(account);
...
async function getMessage(id, account) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  try {
    const g = gmailFor(account);
...
async function sendMessage({ to, subject, body, inReplyTo, references, account }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
...
module.exports = { configured, accounts, resolveAccount, listRecent, getMessage, sendMessage };
