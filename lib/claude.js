...
async function interpret(text, context, facts, nowIsoKST) {
  const system =
    ...
    '{"type":"mail_check","data":{"query":"검색어/조건"|null}} — "메일 확인/새 메일/안 읽은 메일 보여줘". 안 읽은 것만이면 query에 "is:unread".\n' +
    '{"type":"mail_read","data":{"number":정수}} — 직전 메일 목록의 N번 본문을 읽어달라/요약.\n' +
    '{"type":"mail_write","data":{"to":"받는사람"|null,"number":정수|null,"instruction":"보낼 내용/작성 지시"}} — "○○에게 ~내용으로 메일 써줘/보내줘"(새 메일) 또는 "N번 메일에 답장 써줘"(number=답장 대상). 초안만 만들고 바로 보내지 않는다.\n' +
