...
async function createEvent({ userEmail, title, startISO, endISO }) {
...
async function listEvents({ userEmail, fromISO, toISO }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  try {
    const cal = calFor(userEmail);
...
async function updateEvent({ userEmail, eventId, title, startISO, endISO }) {
  if (!configured()) return { ok: false, error: 'not_configured' };
  try {
    const cal = calFor(userEmail);
    const requestBody = {};
